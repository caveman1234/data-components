...
export default {
  ...
  watch:{
    value(value){
      if(this.isEmpty(value) || value === this.selectedValue) return;
      if(this.multiple){
  ...
  beforeMount(){
    if(this.dataSource.length === 0){
      ...
      setTimeout(()=>{
        ...
        var relativeFields = Object.values(this.relative);
        // var fields = [nameField,valueField,...relativeField];
        //需要map的字段
        this.options = options.map(v => {
          var dataSource =  relativeFields.reduce((acc,field) => {
            acc[field] = v[field];
            return acc;
          },{});
          dataSource.label = v[nameField];
          dataSource.value = v[valueField];
          return dataSource;
        });
